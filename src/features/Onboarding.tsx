...
export function Onboarding() {
  ...
  const handleStart = () => {
    ...
    localStorage.setItem("phanh_easy_read", easyRead ? "true" : "false");
    
    // Quick reload to ensure root styles apply if needed (or just navigate, since MissionPlay checks on mount)
    navigate("/missions");
  };
  ...
  return (
    <div className="max-w-3xl mx-auto py-20 px-8">
      <div className="mb-16 text-center border-b border-ink/10 pb-12">
        <div className="text-xs tracking-[0.4em] font-sans uppercase opacity-60 mb-6">Bước 01. Hồ Sơ</div>
        <h1 className="text-5xl font-black font-serif tracking-tighter text-ink mb-6">Xác Định Đối Tượng.</h1>
        <p className="font-sans text-sm tracking-widest uppercase opacity-80 leading-relaxed max-w-xl mx-auto">Chọn một hồ sơ để hệ thống gợi ý kịch bản bám sát rủi ro thực tế.</p>
      </div>
      ...
      <div className="mb-16">
        <label className="flex items-center justify-between p-8 bg-transparent border border-ink/10 cursor-pointer hover:bg-ink/5 transition-colors rounded-none">
          <div className="flex-1 pr-8">
            ...
            <div className="text-[11px] tracking-[0.2em] font-sans uppercase opacity-70">Cỡ chữ lớn, độ tương phản cao, loại bỏ áp lực thời gian.</div>
          </div>
          <div className="flex items-center gap-6">
            ...
            <input 
              ...
              className="w-5 h-5 text-ink border-ink/40 rounded-none focus:ring-0 cursor-pointer"
            />
          </div>
        </label>
      </div>
      ...
      <div className="flex justify-end">
        <button
          ...
          className="bg-ink text-paper px-12 py-5 uppercase tracking-[0.2em] text-[11px] font-bold rounded-none disabled:opacity-30 disabled:cursor-not-allowed hover:bg-ink/80 transition-colors flex items-center gap-4"
        >
